...
export default function EditTestPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/admin/job-openings').then(r => r.json()),
      fetch('/api/admin/questions/stats').then(r => r.json()).catch(() => ({})),
    ]).then(([test, jobs, stats]) => {
      setForm({
        title: test.title,
      ...
      const configs = ((test.assessmentConfig as any[]) || []).map((c: any) => ({
        area: c.area,
        count: c.count,
        easyPct: c.easyPct ?? 30,
        mediumPct: c.mediumPct ?? 50,
        hardPct: c.hardPct ?? 20,
      }))
      setAreas(configs)
      setJobOpenings(jobs)
      setBankStats(stats)
        ...
        <div className="card space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-lg border border-orange-100">
            <input type="checkbox" id="isWalkIn" checked={form.isWalkIn}
              onChange={e => setForm(f => ({ ...f, isWalkIn: e.target.checked, status: e.target.checked ? 'ACTIVE' : f.status }))}
