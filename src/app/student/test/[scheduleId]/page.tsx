...
export default function TestPage() {
  ...
  // Load test data
  useEffect(() => {
    fetch(`/api/student/test/${scheduleId}`)
      .then(r => {
  ...
  function recordViolation(type: string) {
    ...
    setViolations(prev => {
      ...
      // Save to server
      if (attemptId) {
        fetch(`/api/student/test/${scheduleId}/violation`, {
          method: 'POST',
  ...
  function flushSaves() {
    ...
    fetch(`/api/student/test/${scheduleId}/save`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  function selectAnswer(questionId: string, answer: string) {
    setAnswers(prev => {
      ...
      saveTimerRef2.current = setTimeout(flushSaves, 2000)
      return updated
    })
  }
  ...
  async function submitWithRetry(maxRetries = 3): Promise<boolean> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const res = await fetch(`/api/student/test/${scheduleId}/submit`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      <div className="h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="card max-w-lg w-full">
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-800">{testData?.test?.title}</h1>
            <p className="text-gray-500 text-sm mt-1">{testData?.college?.name}</p>
          </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100" onContextMenu={e => e.preventDefault()}>
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-brand-purple text-white shadow-lg">
        <div>
          <div className="font-semibold text-sm">{testData?.test?.title}</div>
